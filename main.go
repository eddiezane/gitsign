...
import (
	...
	cmdSign "github.com/sigstore/gitsign/internal/commands/sign"
)
...
func main() {
	...
	streams := inIO.New(cfg.LogPath)

	rootCmd := root.New(cfg, streams)

	rootCmd.AddCommand(cmdSign.New(cfg, streams))

	err = streams.Wrap(func(s *inIO.Streams) error {
